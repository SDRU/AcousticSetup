...
try:
    user_buffer = 20 * 4096 # 16k, multiple of 4k, always
    ...
    spcm_dwSetParam_i32 (hCard, SPC_CHENABLE,       CHANNEL0)# enable all channels on card
    spcm_dwSetParam_i32 (hCard, SPC_MEMSIZE,        user_buffer)            # acquire 16 kS in total
    spcm_dwSetParam_i32 (hCard, SPC_POSTTRIGGER,    int(user_buffer/2))     # half of the total number of samples after trigger event
    ...
    # spcm_dwSetParam_i32 (hCard, SPC_TRIG_EXT0_LEVEL0, 900)                   # trigger at 1500mV
    # spcm_dwSetParam_i32 (hCard, SPC_TRIG_ANDMASK,     0)                      # ...

    spcm_dwSetParam_i32 (hCard, SPC_CLOCKMODE,      SPC_CM_INTPLL)          # clock mode internal PLL
    # spcm_dwSetParam_i32 (hCard, SPC_TIMESTAMP_CMD, SPC_TSMODE_STANDARD | SPC_TSCNT_INTERNAL | SPC_TSFEAT_NONE) # setting timestamp mode to standard using internal clocking
    ...
    # maxRate = int32 (0)
    # spcm_dwGetParam_i32 (hCard, SPC_PCISAMPLERATE,     byref (maxRate))      # max sampling rate
    ...
    for lChannel in range (0, lSetChannels.value, 1):
        ...
        # spcm_dwSetParam_i64 (hCard, SPC_AMP0,  int32 (5000)) # set input range to +/- 5000 mV
    ...
    qwBufferSize = uint64 (user_buffer * 2 * lSetChannels.value) # in bytes. Enough memory for 16384 samples with 2 bytes each, all channels active
    ...
    # # TIMESTAMP
    # timeBuffer = c_void_p ()
    # spcm_dwDefTransfer_i64 (hCard, SPCM_BUF_TIMESTAMP, SPCM_DIR_CARDTOPC, 0, timeBuffer, 0, qwBufferSize); 
    ...
    # wait until acquisition has finished, then calculated min and max
        ...
        if dwError != ERR_OK:
            ...
            if lBitsPerSample.value <= 8:
                ...
                for i in range (0, user_buffer-1, 1):
                    for lChannel in range (0, lSetChannels.value, 1):
                ...
                for i in range (0, user_buffer-1, 1):                
                    lDataPos = i * lSetChannels.value + alSamplePos[channel]
                    signal=np.append(signal,pnData[lDataPos])
                    for lChannel in range (0, lSetChannels.value, 1):
                        lDataPos = i * lSetChannels.value + alSamplePos[lChannel]
    ...
    plt.plot(np.arange(0,len(signal))*1e-4,signal)
