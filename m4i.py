# -*- coding: utf-8 -*-
"""
Created on Fri Jan 21 17:09:56 2022
...
class M4i(Instrument):
    ...
    def __init__(self, name, cardid='spcm0', **kwargs):
        ...
        self.add_parameter('sample_rate',
                           label='sample rate',
                           get_cmd=partial(self._param64bit,
                                           pyspcm.SPC_SAMPLERATE),
                           unit='Hz',
                           set_cmd=partial(self._set_param64bit,
                                           pyspcm.SPC_SAMPLERATE),
    ...
    def _set_param64bit(self, param, value):
        """ Set a 64-bit parameter on the device."""
        value = int(value)  # convert floating point to int if necessary
        res = pyspcm.spcm_dwSetParam_i64(self.hCard, param, value)
        self._last_set_result = res
        if res == pyspcm.ERR_TIMEOUT:
            logging.warning('SetParam timeout')
        elif res != pyspcm.ERR_OK:
    ...
    #     self._set_param64bit(pyspcm.SPC_SAMPLERATE, rate)
    #     return self._param64bit(pyspcm.SPC_SAMPLERATE)
